...
// Firebase v10+ (modular) with offline persistence + Cloudinary upload + top-right toasts
...
const uploadPreset = "Crrd2025"; // unsigned upload preset
...
/* Enable Firestore IndexedDB offline persistence (Queued writes & local cache) */
...
(async function enablePersistence(){
  ...
  try {
    ...
    console.info("Firestore persistence enabled (IndexedDB).");
...
const state = { items: [], currentFilter: 'all' };
...
function updateOnlineStatus() {
  ...
  if(navigator.onLine) {
    ...
    showToast('Back online — syncing changes', {type:'info', duration:2200});

  } else {

    el.textContent = 'Offline — changes will be saved locally and synced when online.';

    el.style.color = 'var(--danger)';

    showToast('You are offline — changes will sync when online', {type:'info', duration:2200});
...
/* =========================
   ...
   Firestore: realtime listener (reads from cache or server; listens for sync changes)
...
onSnapshot(q, (snapshot) => {
  ...
  // optionally show toast on first load (commented to avoid noisy toasts)

  // showToast('Data updated', {type:'info', duration:1200});

}, (err) => {

  console.error('onSnapshot error', err);

  showToast('Failed to listen to Firestore changes', {type:'error'});
...
qs('#clearAll').addEventListener('click', async ()=> {
  ...
  if(!confirm('Clear ALL data in cloud? This cannot be undone.')) return;

  showToast('Clearing all data (cloud)...', {type:'info', duration:3000});
  ...
  showToast('All data cleared (cloud).', {type:'info'});
...
/* =========================
   ...
   Submit: Add new request (writes are queued offline and synced automatically)
...
qs('#requestForm').addEventListener('submit', async (e) => {
  ...
  // upload image if present (this requires network; if offline, skip and save empty url — you may choose to queue images differently)
  ...
  const item = {
    ...
    usageHistory: [] // array of {date, qty, note}

  };

  try {

    await addDoc(itemsCol, item); // queued when offline; will sync
...
qs('#requestsList').addEventListener('click', async (e) => {
  ...
  if(act === 'delete'){
    ...
    try { await deleteDoc(doc(db, "materialsRequests", id)); showToast('Request deleted', {type:'info'}); } catch(e){ console.error(e); showToast('Failed to delete', {type:'error'}); }
  ...
  if(act === 'approve'){
    ...
    try { await updateDoc(doc(db, "materialsRequests", id), { status: 'Approved' }); showToast('Approved', {type:'success'}); } catch(e){ console.error(e); showToast('Failed', {type:'error'}); }
  ...
  if(act === 'reject'){
    ...
    try { await updateDoc(doc(db, "materialsRequests", id), { status: 'Rejected' }); showToast('Rejected', {type:'info'}); } catch(e){ console.error(e); showToast('Failed', {type:'error'}); }
  ...
  if(act === 'record'){
    ...
    // Keep record even if remaining becomes 0
  ...
  if(act === 'deliver'){
    ...
    // Status: Completed if delivered >= requested
...
/* Remaining actions (from remaining list) */
...
/* Deduct by material across docs (FIFO) */
